...
**要，除非你玩的是 1.20 及以后的版本。**

| 你的版本 | 服务端 |
|---|---|
| 1.20 – 1.21.1 | 不用装。汉化全在客户端读的资源里 |
| 1.19.4 及更早 | **也放一份**，否则导览书《蜜蜂大书》仍是英文 |

因为这几版的导览书在 jar 的 `data/` 里，属于**数据包**，多人游戏时以服务端那份
为准——客户端自己带的不作数。1.15.2 – 1.16.4 还有一批蜂名也在 `data/` 里，
同理。放进服务端的 `mods/` 就行，它在服务端不注册任何东西。
