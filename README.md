...
**单机不用装。** 单机时你的客户端自己兼任逻辑服务端，下面这些都自动生效。

**开服的话，1.21 / 1.21.1 请也放一份到服务端的 `mods/`**；其余版本服务端装了也
没有下面这项功能（那几版蜂笼用的是另一套物品数据 API，没适配）。

| 你看到的东西 | 谁负责 | 服务端不装会怎样 |
|---|---|---|
| 物品、方块、界面、导览书 | 客户端资源 | 正常中文 |
| 世界里蜜蜂头顶的名牌 | 客户端 | 正常中文 |
| 基因样本 / 蜜蜂小食里那行蜂种名 | 客户端 | 正常中文 |
| **蜂笼里那只蜂的名字** | **服务端数据** | **一直是英文** |

蜂笼那行字不是翻译键：蜜蜂被装进蜂笼的那一刻，它的显示名就被解析成一段普通文字
写进了物品里，**谁抓的就按谁的语言表解析**。服务端没有中文，写进去的就是
`Dark Steel Bee`，之后客户端再怎么翻都够不着它——所以只能在服务端做。

装上之后，**以前抓的蜂笼也会自动改回中文**（放进背包里就会改）。改名有三道闸，
你自己起的名字一个都不碰：名字里已经有汉字的不动；当前名字不在「模组自己生成的
英文名」表里的不动（用命名牌起的名字长这样）；改成什么由蜂的 id 决定，不拿英文名
反查。

服务端不装也照样能进服，只是蜂笼名保持英文。本 mod 在服务端只改这一个字段，
不注册方块物品、不动配方、不发网络包。
